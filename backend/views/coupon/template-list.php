...
<div class="" id="card-list" rel="card-list">
    ...
    <div class="pageHeader">
        <form rel="pagerForm" onsubmit="return navTabSearch(this);" action="<?=Url::to(['coupon/template-list'])?>" method="post">
            <div class="searchBar">
                <table class="searchContent">
                    <tbody>
                    <tr>
                        <td>模板名字：<input name="keyword" class="textInput" type="text" alt="模板名字"></td>
                    </tr>
                    </tbody>
                </table>
                <div class="subBar">
    ...
    <div class="pageContent">
        <div class="panelBar">
            <ul class="toolBar">
                <?php if(\Yii::$app->user->can('coupon/edit-template')):?>
                    <li><a class="add" href="<?=Url::to(['coupon/edit-template'])?>" target="navTab"><span>添加</span></a></li>
                <?php endif;?>

                <?php if(\Yii::$app->user->can('coupon/delete-template')):?>
                    <li><a title="确实要删除这些记录吗?" target="selectedTodo" rel="ids[]" href="<?=Url::to(['coupon/delete-template'])?>" class="delete"><span>批量删除</span></a></li>
                <?php endif;?>
        ...
        <table class="table" width="1200" layoutH="138">
            ...
            <tr>
                ...
                <th width="50">模板样式1(未领取)</th>
                <th width="50">模板样式2(已领取)</th>
                <th width="120">金额</th>
            ...
            <?php foreach($dataList as $key => $data):?>
                <tr target="card-id" rel="<?=$data->id?>">
                    ...
                    <td><?=$data->name?></td>
                    <td><img src="<?= \Yii::$app->params['imageUrlPrefix'].$data->style_1 ?>" width="100%"/></td>
                    <td><img src="<?= \Yii::$app->params['imageUrlPrefix'].$data->style_2 ?>" width="100%"/></td>
                    <td><?=$data->sum?></td>
                    ...
                    <td>
                        <?php if(\Yii::$app->user->can('coupon/delete-template')):?>
                            <a title="删除" target="ajaxTodo" href="<?=Url::to(['coupon/delete-template','ids' => $data->id])?>" class="btnDel">删除</a>
                        <?php endif;?>

                        <?php if(\Yii::$app->user->can('coupon/edit-template')):?>
                            <a title="编辑" target="navTab" href="<?=Url::to(['coupon/edit-template','id' => $data->id])?>" class="btnEdit">编辑</a>
                        <?php endif;?>
                    </td>
                </tr>
            <?php endforeach;?>
