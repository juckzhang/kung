...
<div class="" id="card-list" rel="card-list">
    ...
    <div class="pageHeader">
        <form rel="pagerForm" onsubmit="return navTabSearch(this);" action="<?=Url::to(['service/service-order'])?>" method="post">
            <div class="searchBar">
                <table class="searchContent">
                    <tbody>
                    <tr>
                        <td>订单号：<input name="keyword" class="textInput" type="text" alt="订单号"></td>
                    </tr>
                    </tbody>
                </table>
                <div class="subBar">
    ...
    <div class="pageContent">
        <div class="panelBar">
            <ul class="toolBar">
                ...
                <?php if(\Yii::$app->user->can('article/delete-order')):?>
                    <li><a title="确实要删除这些记录吗?" target="selectedTodo" rel="ids[]" href="<?=Url::to(['service/delete-order'])?>" class="delete"><span>批量删除</span></a></li>
                <?php endif;?>
        ...
        <table class="table" width="1200" layoutH="138">
            ...
            <?php foreach($dataList as $key => $data):?>
                <tr target="card-id" rel="<?=$data['id']?>">
                    ...
                    <td>
                        <?php if(\Yii::$app->user->can('service/delete-order')):?>
                            <a title="删除" target="ajaxTodo" href="<?=Url::to(['service/delete-order','ids' => $data['id']])?>" class="btnDel">删除</a>
                        <?php endif;?>

                        <?php if(\Yii::$app->user->can('service/edit-order')):?>
                            <a title="编辑" target="navTab" href="<?=Url::to(['service/edit-order','id' => $data['id']])?>" class="btnEdit">编辑</a>
                        <?php endif;?>
                    </td>
                </tr>
            <?php endforeach;?>
