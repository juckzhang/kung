...
<div class="" id="card-list" rel="card-list">
    ...
    <div class="pageHeader">
        <form rel="pagerForm" onsubmit="return navTabSearch(this);" action="<?=Url::to(['service/service-list'])?>" method="post">
            <div class="searchBar">
                <table class="searchContent">
                    <tbody>
                    <tr>
                        <td>场景标题：<input name="keyword" class="textInput" type="text" alt="场景标题"></td>
                    </tr>
                    </tbody>
                </table>
                <div class="subBar">
    ...
    <div class="pageContent">
        <div class="panelBar">
            <ul class="toolBar">
                <?php if(\Yii::$app->user->can('service/edit-service')):?>
                    <li><a class="add" href="<?=Url::to(['service/edit-service'])?>" target="navTab"><span>添加</span></a></li>
                <?php endif;?>

                <?php if(\Yii::$app->user->can('article/delete-service')):?>
                    <li><a title="确实要删除这些记录吗?" target="selectedTodo" rel="ids[]" href="<?=Url::to(['service/delete-service'])?>" class="delete"><span>批量删除</span></a></li>
                <?php endif;?>
        ...
        <table class="table" width="1200" layoutH="138">
            ...
            <?php foreach($dataList as $key => $data):?>
                <tr target="card-id" rel="<?=$data->id?>">
                    ...
                    <td>
                        <?php if(\Yii::$app->user->can('service/delete-service')):?>
                            <a title="删除" target="ajaxTodo" href="<?=Url::to(['service/delete-service','ids' => $data->id])?>" class="btnDel">删除</a>
                        <?php endif;?>

                        <?php if(\Yii::$app->user->can('service/edit-service')):?>
                            <a title="编辑" target="navTab" href="<?=Url::to(['service/edit-service','id' => $data->id])?>" class="btnEdit">编辑</a>
                        <?php endif;?>
                    </td>
                </tr>
            <?php endforeach;?>
