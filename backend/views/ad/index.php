...
<div class="" id="card-list" rel="card-list">
    ...
    <div class="pageHeader">
        <form rel="pagerForm" onsubmit="return navTabSearch(this);" action="<?=Url::to(['ad/ad-list'])?>" method="post">
            <div class="searchBar">
    ...
    <div class="pageContent">
        <div class="panelBar">
            <ul class="toolBar">
                <?php if(\Yii::$app->user->can('ad/edit-ad')):?>
                <li><a class="add" href="<?=Url::to(['ad/edit-ad'])?>" target="navTab"><span>添加</span></a></li>
                <?php endif;?>

                <?php if(\Yii::$app->user->can('ad/delete-ad')):?>
                <li><a title="确实要删除这些记录吗?" target="selectedTodo" rel="ids[]" href="<?=Url::to(['ad/delete-ad'])?>" class="delete"><span>批量删除</span></a></li>
                <?php endif;?>
        ...
        <table class="table" width="1200" layoutH="138">
            ...
            <tr>
                ...
                <th width="80">按钮名称</th>
                <th width="80">简介</th>
                <th width="180">广告图片</th>
                <th class="<?=$orderDirection?>" tyle="cursor: pointer;" orderfield="start_time" width="80">开始时间</th>
                <th class="<?=$orderDirection?>" tyle="cursor: pointer;" orderfield="end_time" width="80">结束时间</th>
            ...
            <?php foreach($dataList as $key => $data):?>
                <tr target="card-id" rel="<?=$data->id?>">
                    ...
                    <td><img src="<?=\Yii::$app->params['imageUrlPrefix'] . $data->content?>" width="100%"></td>
                    <td><?=date('Y-m-d H:i:s',$data->start_time)?></td>
                    <td><?=date('Y-m-d H:i:s',$data->end_time)?></td>
                    <td><?=date('Y-m-d H:i:s',$data->create_time)?></td>
                    <td>
                        <?php if(\Yii::$app->user->can('ad/delete-ad')):?>
                        <a title="删除" target="ajaxTodo" href="<?=Url::to(['ad/delete-ad','ids' => $data->id])?>" class="btnDel">删除</a>
                        <?php endif;?>

                        <?php if(\Yii::$app->user->can('ad/edit-article')):?>
                        <a title="编辑" target="navTab" href="<?=Url::to(['ad/edit-ad','id' => $data->id])?>" class="btnEdit">编辑</a>
                        <?php endif;?>
                    </td>
                </tr>
            <?php endforeach;?>
