...
<div class="" id="video-list" rel="video-list">
    ...
    <div class="pageHeader">
        <form id = "adrelforms" rel="pagerForm" onsubmit="return dialogSearch(this);" action="<?= Url::to(['video/delvideo-list']) ?>"
              method="post">
            ...
            <div class="searchBar">
                ...
                <div class="subBar">
                    <ul>
                        ...
                        <li><div class="button"><div class="buttonContent"><button type="button" onclick="addToalbum(<?=$album?>);" warn="选择视频">从专辑中移除</button></div></div></li>
        ...
        <table class="table" width="512" layoutH="138">
            ...
            <?php foreach ($dataList as $key => $data): ?>
                <tr target="video-id" rel="<?= $data['id'] ?>">
                    ...
                    <td><input class="sort_order" onchange="sort(this)" type="text" name="sort_order" data-id="<?= $data['id']?>" value="<?= $data['sort_order']?>"></td>
                    <td><?= date('Y-m-d', $data['create_time']) ?></td>
                    <td>
                        <?php if (\Yii::$app->user->can('video/toalbum')): ?>
                            <a title="加入专辑"  onclick="addToalbums(<?=$album?>,<?=$data['id']?>);"  class="btnDel">加入专辑</a>
                        <?php endif; ?>
                    </td>
                </tr>
            <?php endforeach; ?>
...
<script type="text/javascript">
   ...
   function sort(obj) {
       var videoid = $(obj).data('id');
       var sort = $(obj).val();

       $.ajax({
           url:'<?=Url::to(['video/sort'])?>',
           dataType: 'json',//返回数据的类型
           data:{videoid:videoid,sort:sort},
           success: function (result, status) {
               console.log(result);
               if(result.statusCode == 200)
               {
