...
<div class="pageHeader">
    <form rel="pagerForm" onsubmit="return navTabSearch(this);" action="<?=Url::to(['system/source-list'])?>" method="post">
    </form>
</div>
<div class="pageContent">
    <div class="panelBar">
        <ul class="toolBar">
            ...
            <li><a class="add" href="<?=Url::to(['system/edit-source'])?>" target="dialog"><span>添加</span></a></li>
            <?php endif;?>

            <?php if(\Yii::$app->user->can('system/delete-source')):?>
            <li><a title="确实要删除这些记录吗?" target="selectedTodo" rel="ids[]" href="<?=Url::to(['system/delete-source'])?>" class="delete"><span>批量删除</span></a></li>
            <?php endif;?>
...
<!--            <li><a class="icon" href="demo/common/dwz-team.xls" target="dwzExport" targetType="navTab" title="实要导出这些记录吗?"><span>导出EXCEL</span></a></li>-->
    ...
    <table class="table" width="1200" layoutH="138">
        ...
        <?php foreach($dataList as $key => $data):?>
            <tr target="card-id" rel="<?=$data->id?>">
                ...
                <td>
                    <?php if(\Yii::$app->user->can('system/delete-source')):?>
                    <a title="删除" target="ajaxTodo" href="<?=Url::to(['system/delete-source','ids' => $data->id])?>" class="btnDel">删除</a>
                    <?php endif;?>

                    <?php if(\Yii::$app->user->can('system/edit-source')):?>
                    <a title="编辑" target="dialog" href="<?=Url::to(['system/edit-source','id' => $data->id])?>" class="btnEdit">编辑</a>
                    <?php endif;?>
                </td>
            </tr>
        <?php endforeach;?>
    ...
    <div class="panelBar">
        ...
        <div class="pagination" rel='source-list' targetType="navTab" totalCount="<?=$dataCount?>" numPerPage="<?=$prePage?>" pageNumShown="10" currentPage="<?=$page?>"></div>
    </div>
</div>
</div>
