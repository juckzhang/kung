...
//查询条件
$positionId = ArrayHelper::getValue($params,'positionId');
$positions = [
...
<div class="pageHeader">
    <form rel="pagerForm" onsubmit="return navTabSearch(this);" action="<?=Url::to(['menu/menu-list'])?>" method="post">
        <div class="searchBar">
                    ...
                    <td>
                        <select class="combox" name="positionId">
                            <?php foreach($positions as $key => $value):?>
                                <option value="<?=$key?>" <?=$positionId == $key ? 'selected' :''?>><?=$value?></option>
                            <?php endforeach;?>
                        </select>
                    </td>
                </tr>
...
<div class="pageContent">
    <div class="panelBar">
        <ul class="toolBar">
            <?php if(\Yii::$app->user->can('menu/edit-menu')):?>
            <li><a class="add" href="<?=Url::to(['menu/edit-menu'])?>" target="dialog"><span>添加</span></a></li>
            <?php endif;?>

            <?php if(\Yii::$app->user->can('menu/delete-menu')):?>
            <li><a title="确实要删除这些记录吗?" target="selectedTodo" rel="ids[]" href="<?=Url::to(['menu/delete-menu'])?>" class="delete"><span>批量删除</span></a></li>
            <?php endif;?>
    ...
    <table class="table" width="1200" layoutH="138">
        ...
        <?php foreach($dataList as $key => $data):?>
            <tr target="card-id" rel="<?=$data->id?>">
                ...
                <td>
                    <?php if(\Yii::$app->user->can('menu/delete-menu')):?>
                    <a title="删除" target="ajaxTodo" href="<?=Url::to(['menu/delete-menu','ids' => $data->id])?>" class="btnDel">删除</a>
                    <?php endif;?>

                    <?php if(\Yii::$app->user->can('menu/edit-menu')):?>
                    <a title="编辑" target="dialog" href="<?=Url::to(['menu/edit-menu','id' => $data->id])?>" class="btnEdit">编辑</a>
                    <?php endif;?>
                </td>
            </tr>
        <?php endforeach;?>
