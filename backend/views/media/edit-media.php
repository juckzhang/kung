...
<div class="pageContent">
    <form method="post" action="<?=Url::to(['media/edit-media','id' => ArrayHelper::getValue($model,'id','')])?>" class="pageForm required-validate" onsubmit="return validateCallback(this,navTabAjaxDone)">
        <div class="pageFormContent nowrap" layoutH="97">
            <p>
                <label>专辑：</label>
                <input type="hidden" id='album-id' name="mediaModel[album_id]" data-name="album.id" value="<?=ArrayHelper::getValue($model,'album_id')?>">
                ...
                <a class="btnLook" href="<?=Url::to(['media/album-list','search' => 1])?>" lookupgroup="album">查找带回</a>
            </p>
            <p>
                <label>视频名称：</label>
            ...
            <p>
                <label>唯一标识符号：</label>
                <input id="vu" type="text" name="mediaModel[media_unique]" value="<?=ArrayHelper::getValue($model,'media_unique')?>">
            </p>
            <p>
                <label>乐视用户凭证：</label>
                <input id="uu" type="text" name="mediaModel[leshi_user]" value="<?=ArrayHelper::getValue($model,'leshi_user')?>">
            </p>
            <p>
                <label>视频id：</label>
                <input id="media_id" type="text" name="mediaModel[media_id]" value="<?=ArrayHelper::getValue($model,'media_id')?>">
            </p>
            <p>
                <label>视频链接：</label>
                <input type="text" name="mediaModel[media_link]" value="<?=ArrayHelper::getValue($model,'media_link')?>">
            </p>
            <p>
                <label>点击量：</label>
                <input type="text" name="mediaModel[click_num]" value="<?=ArrayHelper::getValue($model,'click_num',0)?>">
            </p>
            <p>
                <label>所属用户：</label>
                <select name="mediaAlbumModel[user_id]">
            ...
            <div class="album">
                <p>
                    <label>视频分类：</label>
                    <select name="mediaAlbumModel[cate_id]" value="<?=$cateId?>">
                ...
                <p>
                    <label>专辑名称：</label>
                    <input type="text" name="mediaAlbumModel[name]" value="<?=ArrayHelper::getValue($album,'name')?>">
                    <!-- 专辑id -->
                    <input type="hidden" name="albumId" value="<?=ArrayHelper::getValue($album,'id')?>">
                    <input type="hidden" name="mediaAlbumModel[has_media]" value=1>
                </p>
                <p>
                    <label>导演：</label>
                    <input type="text" name="mediaAlbumModel[director]" value="<?=ArrayHelper::getValue($album,'director')?>">
                </p>
                <p>
                    <label>演员：</label>
                    <input type="text" name="mediaAlbumModel[actor]" value="<?=ArrayHelper::getValue($album,'actor')?>">
                </p>
                <p>
                    <label>地区：</label>
                    <select name="mediaAlbumModel[area]">
                ...
                <p>
                    <label>发行年份：</label>
                    <select name="mediaAlbumModel[release_time]">
                        <?php for($i = date('Y') ; $i >= 1970; --$i):?>
                            <option value="<?=$i?>" <?=ArrayHelper::getValue($album,'release_time',1970) == $i ? 'selected="selected"' : ''?>><?=$i?></option>
                        <?php endfor;?>
                    </select>
<!--                    <input type="text" name="mediaAlbumModel[release_time]" value="<=ArrayHelper::getValue($album,'release_time')?>">-->
                </p>
<!--                <p>-->
<!--                    <label>是否轮播推荐：</label>-->
<!--                    <select name="mediaAlbumModel[is_carousel]" value="0">-->
...
<!--                </p>-->
                <p>
                    <label>是否楼层推荐：</label>
                    <select name="mediaAlbumModel[is_recommend]" value="<?=ArrayHelper::getValue($album,'is_recommend',0)?>">
                ...
                <p>
                    <label>是否最热推荐：</label>
                    <select name="mediaAlbumModel[is_hot]" value="<?=ArrayHelper::getValue($album,'is_hot',0)?>"">
                        <option value="0">不推荐</option>
                ...
                <p>
                    <label>是否最新推荐：</label>
                    <select name="mediaAlbumModel[is_new]" value="<?=ArrayHelper::getValue($album,'is_new',0)?>"">
                        <option value="0">不推荐</option>
                ...
                <p>
                    <label>一句话简介：</label>
                    <input type="text" name="mediaAlbumModel[aword]" value="<?=ArrayHelper::getValue($album,'aword')?>">
                </p>
                <p>
                    <label>视频标签：</label>
                    <input type="text" name="mediaModel[tags]" value="<?=ArrayHelper::getValue($model,'tags')?>">
                </p>
                <p>
                    <label>播放时长：</label>
                    <input type="text" name="mediaModel[play_time]" value="<?=ArrayHelper::getValue($model,'play_time')?>">
                </p>
                <dl>
                    <dt>在 App 内隐藏：</dt>
                    <dd>
                        <select name="mediaAlbumModel[api_hidden]" value="<?=ArrayHelper::getValue($album,'api_hidden',0)?>"">
                ...
                <dl>
                    <dt>简介：</dt>
                    <dd>
                        <textarea style="width: 510px;" name="mediaAlbumModel[introduction]" value="<?=ArrayHelper::getValue($album,'introduction','')?>"><?=ArrayHelper::getValue($album,'introduction','')?></textarea>
                    </dd>
                </dl>
                <p>
                    <label>横板海报图片：</label>
                    <input type="text" name="mediaAlbumModel[wide_poster]" class='wide-poster' value="<?=ArrayHelper::getValue($album,'wide_poster','')?>"/>
                </p>
                <p>
                    <label>竖版海报图片：</label>
                    <input type="text" name="mediaAlbumModel[poster_url]" class='poster-url' value="<?=ArrayHelper::getValue($album,'poster_url','')?>"/>
                </p>
                <p>
                    <label>&nbsp;</label>
                ...
                <dl style="margin-top:100px;">
                    ...
                    <label>平台账号类型：</label>
                    <select id="service-select" name="mediaModel[service_num]" value="<?=ArrayHelper::getValue($model,'service_num',0)?>"">
