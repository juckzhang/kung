...
<div class="pageHeader">
    <form rel="pagerForm" onsubmit="return navTabSearch(this);" action="<?=Url::to(['article/category-list'])?>" method="post">
        <div class="searchBar">
...
<div class="pageContent">
    <div class="panelBar">
        <ul class="toolBar">
            <?php if(\Yii::$app->user->can('article/edit-category')):?>
            <li><a class="add" href="<?=Url::to(['article/edit-category'])?>" target="dialog"><span>添加</span></a></li>
            <?php endif;?>

            <?php if(\Yii::$app->user->can('article/delete-category')):?>
            <li><a title="确实要删除这些记录吗?" target="selectedTodo" rel="ids[]" href="<?=Url::to(['article/delete-category'])?>" class="delete"><span>批量删除</span></a></li>
            <?php endif;?>
    ...
    <table class="table" width="1200" layoutH="138">
        ...
        <?php foreach($dataList as $key => $data):?>
            <tr target="card-id" rel="<?=$data->id?>">
                ...
                <td>
                    <?php if(\Yii::$app->user->can('article/edit-category')):?>
                    <a title="删除" target="ajaxTodo" href="<?=Url::to(['article/delete-category','ids' => $data->id])?>" class="btnDel">删除</a>
                    <?php endif;?>

                    <?php if(\Yii::$app->user->can('article/edit-category')):?>
                    <a title="编辑" target="dialog" href="<?=Url::to(['article/edit-category','id' => $data->id])?>" class="btnEdit">编辑</a>
                    <?php endif;?>
                </td>
            </tr>
        <?php endforeach;?>
